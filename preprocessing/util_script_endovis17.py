...
base_dir = "/mnt/sdb/zhaoshiyu/timesam_endovis17_fold1"
...
def organize_data():
    save_pth = base_dir + '/' + 'Dataset907_endovis17'

    os.makedirs(save_pth+'/imagesTr', exist_ok=True)
    os.makedirs(save_pth+'/labelsTr', exist_ok=True)

    data_pth_all = [base_dir + '/' + 'endovis17/Train',
                    base_dir + '/' + 'endovis17/Test',
                    ]
    ...
    for data_pth in data_pth_all:
        ...
        for data_fd in data_fd_list:
            ...
            for filename in filename_list:
                ...
                jpgname = filename.replace(".png", ".jpg")

                shutil.copy(data_pth + '/' + data_fd + '/left_frames' + '/' + jpgname, save_pth+'/imagesTr/endovis_'+patient_ID+file_ID+'_0000.png')


def get_all_5slice():

    save_pth = base_dir + '/endovis17/2D_all_5slice'
    data_pth_all = [base_dir + '/Dataset907_endovis17',
                    ]
...
def get_csv():
    ...
    test_fd_list = ['0021', '0022']

    training_fd_list = list(set(data_fd_list)-set(test_fd_list))
    validation_fd_list = random.sample(test_fd_list, 2)
