...
def test_single_volume(image, label, net, classes, multimask_output, patch_size=[512, 512], test_save_path=None, case=None):
    ...
    for ind in range(image.shape[0]):
        ...
        with torch.no_grad():
            ...
            net.to(device)
            if isinstance(inputs[0], dict) and 'image' in inputs[0]:
                # 如果 inputs 是 [{'image': tensor}, ...]
                inputs_tensor = torch.stack([x['image'].to(device) for x in inputs], dim=0)
            else:
                # 如果 inputs 是 [tensor, tensor, ...]
                inputs_tensor = torch.stack([x.to(device) for x in inputs], dim=0)

            # FLOPs 分析
            from fvcore.nn import FlopCountAnalysis
            with torch.no_grad():
                flops = FlopCountAnalysis(net, (inputs_tensor, multimask_output, patch_size[0]))
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--adapt_ckpt', type=str, default='/mnt/sdb/zhaoshiyu/project_results/TDSAM_endovis18_8/result_20241211/epoch_394.pth', help='The checkpoint after adaptation')
    # parser.add_argument('--adapt_ckpt', type=str, default='/mnt/sdb/zhaoshiyu/project_results/TDSAM_endovis18_12/result_20241211/epoch_399.pth', help='The checkpoint after adaptation')
    parser.add_argument('--adapt_ckpt', type=str, default='/mnt/sdb/zhaoshiyu/project_results/TDSAM_endovis17/result_fold3_20250106/epoch_299.pth', help='The checkpoint after adaptation')
    # parser.add_argument('--data_path', type=str, default='/mnt/sdb/zhaoshiyu/timesam_endovis18/endovis18/2D_all_5slice')
    # parser.add_argument('--data_path', type=str, default='/mnt/sdb/zhaoshiyu/endovis18/2D_all_5slice')
    parser.add_argument('--data_path', type=str, default='/mnt/sdb/zhaoshiyu/endovis17_fold3_sam2/endovis17/2D_all_5slice')
    # parser.add_argument('--data_path', type=str, default='/mnt/sdb/zhaoshiyu/NAL_endovis/2D_all_5slice')
    ...
    parser.add_argument('--ckpt', type=str, default='/home/zhaoshiyu/TDSAM_20241106/sam_vit_b_01ec64.pth', help='Pretrained checkpoint')
    parser.add_argument('--vit_name', type=str, default='vit_b', help='Select one vit model')
    parser.add_argument('--rank', type=int, default=128, help='Rank for FacT adaptation')
    ...
    # + "_endovis18_12"
    # args.output_dir = args.adapt_ckpt[:-4] + "_nal_endovis"
