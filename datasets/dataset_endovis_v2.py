...
def random_erasing(
    ...
    filling = np.repeat(filling, imgshape[-2], axis=-1)
    filling = filling[:,:,:,np.newaxis]
...
def posterize(img, label, v):
    ...
    for slice_indx in range(img.shape[-1]):
...
def adjust_light(image, label):
    ...
    for slice_indx in range(image.shape[-1]):
...
def shear_x(img, label, v):
    ...
    for slice_indx in range(label.shape[-1]):
...
class RandomGenerator(object):
    ...
    def __call__(self, sample):
        ...
        if x != self.output_size[0] or y != self.output_size[1]:
            image = zoom(image, (self.output_size[0] / x, self.output_size[1] / y, 1.0, 1.0), order=3)
...
class dataset_reader(Dataset):
    def __init__(self, base_dir, split, num_classes, transform=None):
        ...
        self.samples= self.preprocess_func()
